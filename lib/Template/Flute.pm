...
templating language for your designers to learn - instead, standard HTML and
CSS classes are used, leading to HTML that can easily be understood and edited
by WYSIWYG editors and hand-coding designers alike.
...
previews in WYSWYG tools, CSS selectors in the template are tied to your data
structures or objects by a specification provided by the programmer.
...
L<Template::Flute::Specification::XML> or
L<Template::Flute::Specification::Scoped>.).
...
Select specification parser. This can be either the full class name like
L<MyApp::Specification::Parser> or the last part for classes residing in the
Template::Flute::Specification namespace.
...
sub new {
        ...
        && !ref( $self->{specification} ) )
    {
        ...
        $self->_bootstrap_specification( 'string',
            delete $self->{specification} );
    }
...
sub _bootstrap {
    my ($self) = @_;
    my ( $parser_name, $parser_spec, $spec_file, $spec, $template_file,
        $template_object );
    ...
    unless ( $self->{specification} ) {
        unless ( $self->{specification_file} ) {
            ...
            unless ( -f $self->{specification_file} ) {
                die
                  "Missing Template::Flute specification for template $self->{template_file}\n";
            }
        }
...
sub _bootstrap_specification {
    ...
    if ( $parser_name = $self->{specification_parser} ) {
        ...
        eval { $parser_spec = $class->new(); };

        if ($@) {
            die
              "Failed to instantiate class $class as specification parser: $@\n";
        }
    }
    else {
    ...
    if ( $source eq 'file' ) {
        unless ( $self->{specification} =
            $parser_spec->parse_file($specification) )
        {
            die "$0: error parsing $specification: "
              . $parser_spec->error() . "\n";
        }
    }
    else {
...
Processes HTML template, manipulates the HTML tree based on the specification,
    ...
    for my $list ( $self->{template}->lists() ) {
        ...
        unless ( $iter = $list->iterator() ) {
            if ( $name = $list->iterator('name') ) {
                ...
                if ( $iter = $self->{specification}->iterator($name) ) {
                    ...
                    die "Missing iterator object for list "
                      . $list->name
                      . " and iterator name $name";
                }
            }
            elsif ( $self->{database} ) {
                if ( $query = $list->query() ) {
                    ...
                    die "List "
                      . $list->name
                      . " without iterator and database query.\n";
                }
            }
            else {
                die "List "
                  . $list->name
                  . " without iterator and database object.\n";
            }
        }
        ...
        while ( $row = $iter->next() ) {
            ...
            if ( $row = $list->filter( $self, $row ) ) {
                $list_elt =
                  $self->_replace_record( $list, 'list', $lel, \%paste_pos,
                    $row, $row_pos );
            ...
            if ( exists $param->{op} && $param->{op} eq 'append' ) {
                if ( exists $param->{joiner} ) {
                    $elt->set_att( $zref->{rep_att},
                        $zref->{rep_att_orig} . $param->{joiner} . $rep_str );
                }
                else {
                    $elt->set_att( $zref->{rep_att},
                        $zref->{rep_att_orig} . $rep_str );
                }
            }
...
sub _replace_record {
    ...
    my (
        $key,      $filter,       $rep_str,      $att_name,
        $att_spec, $att_tag_name, $att_tag_spec, %att_tags,
        $att_val,  $class_alt,    $elt_handler
    );
...
sub filter {
    ...
    unless ( exists $self->{_filter_objects}->{$name} ) {
        ...
        eval {
            $filter_obj =
              $class->new( options => $self->{_filter_opts}->{$name} );
        };
...
sub value {
    ...
    if ( exists $value->{include} ) {
        ...
        if ( $self->{template_file} ) {
            $include_file =
              Template::Flute::Utils::derive_filename( $self->{template_file},
                $value->{include}, 1, pass_absolute => 1 );
        }
        else {
...
Sets hash reference of values to be used by the process method. Same as passing
the hash reference as values argument to the constructor.
...
Returns HTML template object, see L<Template::Flute::HTML> for details.
...
format implemented by L<Template::Flute::Specification::Scoped> module or write
your own specification parser class.
...
The first container is only shown in the output if the value C<billing_address>
is set:

  <container name="billing" value="billing_address" class="billingWrapper">
  </container>

The second container is shown if the value C<warnings> or the value C<errors>
is set:
...
Param elements are replaced with the corresponding value from the list
iterator.
...
Processes the template file named in this attribute. This implies the hook
operation.
...
Form elements are tied through specification to HTML forms. Attributes for form
elements in addition to C<class> and C<id> are:

=over 4

=item link

The link attribute can only have the value C<name> and allows to base the
relationship between form specification elements and HTML form tags on the name
HTML attribute instead of C<class>, which is usually more convenient.
...
the document tree. This abstraction relieves us from worrying about where the
data actually comes from. We basically just need an array of hash references
and an iterator class with a next and a count method. For your convenience you
can create an iterator from L<Template::Flute::Iterator> class very easily.
...
Only lists present in the specification and the HTML template can be addressed
in this way.
...
Only forms present in the specification and the HTML template can be addressed
in this way.
...
Filters are used to change the display of value and param elements in the
...
L<Template::Flute::Filter::NobreakSingle>.

=item currency

Currency filter, see L<Template::Flute::Filter::Currency>. Requires
L<Number::Format> module.

=item date

Date filter, see L<Template::Flute::Filter::Date>. Requires L<DateTime> and
L<DateTime::Format::ISO8601> modules.

=item country_name

Country name filter, see L<Template::Flute::Filter::CountryName>. Requires
L<Locales> module.

=item language_name

Language name filter, see L<Template::Flute::Filter::LanguageName>. Requires
L<Locales> module.

=back

Filter classes are loaded at runtime for efficiency and to keep the number of
dependencies for Template::Flute as small as possible.
...
because of a request from Matt S. Trout, author of the L<HTML::Zoom> module.
